...
public class ItemRestController {
	...
	private ItemService service ; // injected
	 ...
	 */
	@GetMapping("")
	protected ResponseEntity<List<ItemDTO>> findAll() {
     ...
     */
    @GetMapping("/{id}")
    protected ResponseEntity<ItemDTO> findById(@PathVariable int id) {
    	logger.debug("REST : GET - findById");
    	ItemDTO itemDTO = service.findById(id);
		if ( itemDTO != null ) {
			return ResponseEntity.ok(itemDTO); // 200 OK, found
		}
		else {
	 ...
	 */
	@PostMapping("")
	protected ResponseEntity<Void> create(@RequestBody ItemDTO itemDTO) {
    	logger.debug("REST : POST - create");
		if ( service.create(itemDTO) ) {
	 ...
	 */
	@PutMapping("/{id}")
	protected ResponseEntity<Void> save(@PathVariable int id, @RequestBody ItemDTO itemDTO) {
    	logger.debug("REST : PUT - save");
		service.save(id, itemDTO);
	 ...
	 */
	@PutMapping("")
	protected ResponseEntity<Void> update(@RequestBody ItemDTO itemDTO) {
    	logger.debug("REST : PUT - update");
		if ( service.update(itemDTO) ) {
	 ...
	 */
	@PatchMapping("/{id}")
	protected ResponseEntity<Void> partialUpdate(@PathVariable int id, @RequestBody ItemDTO itemDTO) {
    	logger.debug("REST : PATCH - partialUpdate");
    	if ( service.partialUpdate(id, itemDTO) ) {
