...
import com.diospyros.uplift.services.EntityService;

@RestController
@RequestMapping(value = "/api/v1/entity", produces = MediaType.APPLICATION_JSON_VALUE)
public class EntityRestController {

	private static final Logger logger = LoggerFactory.getLogger(EntityRestController.class);
	
	private EntityService service ; // injected
	 ...
	 * @param service
	 */
	protected EntityRestController(EntityService service) {
		super();
	 ...
	 */
	@GetMapping("")
	protected ResponseEntity<List<EntityDTO>> findAll() {
    	logger.debug("REST : GET - findAll");
    	List<EntityDTO> list = service.findAll();
     ...
     */
    @GetMapping("/{id}")
    protected ResponseEntity<EntityDTO> findById(@PathVariable int id) {
    	logger.debug("REST : GET - findById");
    	EntityDTO entityDTO = service.findById(id);
		if ( entityDTO != null ) {
			return ResponseEntity.ok(entityDTO); // 200 OK, found
		}
		else {
	 ...
	 */
	@PostMapping("")
	protected ResponseEntity<Void> create(@RequestBody EntityDTO entityDTO) {
    	logger.debug("REST : POST - create");
		if ( service.create(entityDTO) ) {
	 ...
	 */
	@PutMapping("/{id}")
	protected ResponseEntity<Void> save(@PathVariable int id, @RequestBody EntityDTO entityDTO) {
    	logger.debug("REST : PUT - save");
		service.save(id, entityDTO);
	 ...
	 */
	@PutMapping("")
	protected ResponseEntity<Void> update(@RequestBody EntityDTO entityDTO) {
    	logger.debug("REST : PUT - update");
		if ( service.update(entityDTO) ) {
	 ...
	 */
	@PatchMapping("/{id}")
	protected ResponseEntity<Void> partialUpdate(@PathVariable int id, @RequestBody EntityDTO entityDTO) {
    	logger.debug("REST : PATCH - partialUpdate");
    	if ( service.partialUpdate(id, entityDTO) ) {
