...
public class LocationRestController {
	...
	private LocationService service ; // injected
	 ...
	 * @param service
	 */
	protected LocationRestController(LocationService service) {
		super();
	 ...
	 */
	@GetMapping("")
	protected ResponseEntity<List<LocationDTO>> findAll() {
     ...
     */
    @GetMapping("/{id}")
    protected ResponseEntity<LocationDTO> findById(@PathVariable int id) {
    	logger.debug("REST : GET - findById");
    	LocationDTO locationDTO = service.findById(id);
		if ( locationDTO != null ) {
			return ResponseEntity.ok(locationDTO); // 200 OK, found
		}
		else {
	 ...
	 */
	@PostMapping("")
	protected ResponseEntity<Void> create(@RequestBody LocationDTO locationDTO) {
    	logger.debug("REST : POST - create");
		if ( service.create(locationDTO) ) {
	 ...
	 */
	@PutMapping("/{id}")
	protected ResponseEntity<Void> save(@PathVariable int id, @RequestBody LocationDTO locationDTO) {
	 ...
	 */
	@PutMapping("")
	protected ResponseEntity<Void> update(@RequestBody LocationDTO locationDTO) {
    	logger.debug("REST : PUT - update");
		if ( service.update(locationDTO) ) {
	 ...
	 */
	@PatchMapping("/{id}")
	protected ResponseEntity<Void> partialUpdate(@PathVariable int id, @RequestBody LocationDTO locationDTO) {
    	logger.debug("REST : PATCH - partialUpdate");
    	if ( service.partialUpdate(id, locationDTO) ) {
