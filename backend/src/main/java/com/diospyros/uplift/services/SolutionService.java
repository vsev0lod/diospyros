...
/**
 * REST service for entity "Solution" <br>
 * 
 ...
 */
@Service
public class SolutionService extends GenericService<Solution, SolutionDTO> {

	private static final Logger logger = LoggerFactory.getLogger(SolutionService.class);

	private final SolutionRepository repository; // injected by constructor
	...
	public SolutionService(SolutionRepository repository) {
		super(Solution.class, SolutionDTO.class);
	 ...
	 * @return
	 */
	private Integer getEntityId(SolutionDTO dto) {
		return dto.getId();
	}
	...
	public List<SolutionDTO> findAll() {
		...
		Iterable<Solution> all = repository.findAll();
	 ...
	 */
	public SolutionDTO findById(int id) {
		Integer entityId = id;
		logger.debug("findById({})", entityId);
		Optional<Solution> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(int id, SolutionDTO dto) {
		Integer entityId = id;
	 ...
	 */
	public boolean update(SolutionDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(int id, SolutionDTO dto) {
		Integer entityId = id;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		Optional<Solution> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Solution entity = optionalEntity.get();
	 ...
	 */
	public boolean create(SolutionDTO dto) {
	...
	// -----------------------------------------------------------------------------------------
/***
	public List<SolutionDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		// List<Solution> list = repository.findByTitle(title);
		List<Solution> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<SolutionDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		// List<Solution> list = repository.findByTitle(title);
		List<Solution> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}

	public List<SolutionDTO> findByTitleAndPrice(String title, BigDecimal price) {
		logger.debug("findByTitleAndPrice({}, {})", title, price);
		List<Solution> list = repository.findByTitleContainingAndPrice(title, price);
		return entityListToDtoList(list);
	}
***/
}
